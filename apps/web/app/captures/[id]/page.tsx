...
export default function CaptureDetailPage({ params }: { params: { id: string } }) {
  ...
  // EXPENSE classify fields
  const [expenseAccountId, setExpenseAccountId] = useState('');
  const [paymentAccountId, setPaymentAccountId] = useState('');
  // REVENUE classify fields
  ...
  useEffect(() => {
    ...
    Promise.all([api.getCapture(params.id), api.listAccounts(), api.getSettings()])
      .then(([c, a, s]) => {
        ...
        if (c.appliedRevenueRule) {
          setRevenueAccountId(c.appliedRevenueRule.revenueAccount.id);
          setReceivingAccountId(c.appliedRevenueRule.receivingAccount.id);
        }
      })
  ...
  const isRevenue = capture?.type === 'REVENUE';
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      if (isRevenue) {
        await api.classifyRevenueCapture(capture.id, { revenueAccountId, receivingAccountId, saveAsRule });
      } else {
        ...
        {!editing ? (
          <>
            <div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <h2 style={{ margin: 0 }}>
                {isRevenue && <span className="badge posted" style={{ marginRight: 8 }}>Revenue</span>}
                {capture.description}
              </h2>
              {canEdit && (
            ...
            {capture.customerName && <p className="subtitle">Customer: {capture.customerName}</p>}
            ...
            {capture.appliedRevenueRule && (
              <p className="subtitle">
                Suggested (from a matching rule): {capture.appliedRevenueRule.receivingAccount.code} ·{' '}
                {capture.appliedRevenueRule.receivingAccount.name} / {capture.appliedRevenueRule.revenueAccount.code} ·{' '}
                {capture.appliedRevenueRule.revenueAccount.name} — still requires your confirmation.
              </p>
            )}
          </>
        ) : (
        ...
        <div className="card">
          <form onSubmit={onSubmit}>
            {isRevenue ? (
              <>
                <label>
                  Debit account (where it was received)
                  <select required value={receivingAccountId} onChange={(e) => setReceivingAccountId(e.target.value)}>
                ...
                <label>
                  ...
                  <select required value={revenueAccountId} onChange={(e) => setRevenueAccountId(e.target.value)}>
                    <option value="" disabled>
                      Select account
                    </option>
                    {accounts.map((a) => (
